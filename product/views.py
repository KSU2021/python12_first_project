...
# SELECT * FROM product WHERE title LIKE 'Apple%';-  зависит от регистра
# Product.objects.filter(title__istartswith='Apple')
# SELECT * FROM product WHERE title ILIKE 'Apple%'; - не зависит от регистра

'%work'
# Product.objects.filter(title__endswith='GB')- зависит от регистра
# SELECT * FROM product WHERE title LIKE '%GB';
# Product.objects.filter(title__iendswith='GB') - не зависит от регистра
...
# MultipleObjectReturned - возникает, когда найдено больше одного объекта
#
...
#QuerySet - список объектов модели по запросу из базы данных
